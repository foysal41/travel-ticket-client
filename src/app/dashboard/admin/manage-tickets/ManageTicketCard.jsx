...
import { updateTicketStatus } from '@/app/lib/actions/updateTicketStatus';
import { toast } from 'react-toastify';
import { useRouter } from "next/navigation";

const ManageTicketCard = ({tickets}) => {
    const router = useRouter();
   

    const handleApprove = async(id) =>{
        const res = await updateTicketStatus(id, "approved");
    ...
    const res = await updateTicketStatus(id, "rejected");
                ...
                >
                  ...
                  <td className="px-4 py-4">
                    <div className="flex flex-row gap-2">
                      <Button  onClick={() => handleApprove(ticket._id)} className="h-9 rounded-lg border border-blue-500 bg-white px-4 text-sm text-blue-600 hover:bg-green-50">
                        Approve
                      </Button>

                      <Button  onClick={() => handleReject(ticket._id)} className="h-9 rounded-lg border border-red-500 bg-white px-4 text-sm text-red-500 hover:bg-red-50">
                        Reject
                      </Button>
                    </div>
                  </td>
